...
jhhrvnonrddetailUI <- function(tabTitle ='非研发工时',
...
buttonnonrddetailUI_left <- function() {
  ...
  res <- tagList(
    ...
    shiny::actionButton(inputId = 'btn_hrv_ods_view_nonrddetail' , label = '预览表单数据'),
...
buttonnonrddetailUI_right <- function() {
  res <- tagList(
    tsui::uiScrollX(tsui::mdl_dataTable(id = 'hrv_ods_view_data_nonrddetail', label = '出口'))
  )
  return(res)
