...
jhhrvworkcenterUI <- function(tabTitle ='责任中心',
...
buttonworkcenterUI_left <- function() {
  ...
  res <- tagList(
    ...
    shiny::actionButton(inputId = 'btn_hrv_ods_view_workcenter' , label = '预览表单数据'),
...
buttonworkcenterUI_right <- function() {
  res <- tagList(
    tsui::uiScrollX(tsui::mdl_dataTable(id = 'hrv_ods_view_data_workcenter', label = '出口'))
  )
  return(res)
