...
jhhrvcosucompanyUI <- function(tabTitle ='往来单位',
...
buttoncosucompanyUI_left <- function() {
  ...
  res <- tagList(
    ...
    shiny::actionButton(inputId = 'btn_hrv_ods_view_cosucompany' , label = '预览表单数据'),
...
buttoncosucompanyUI_right <- function() {
  res <- tagList(
    tsui::uiScrollX(tsui::mdl_dataTable(id = 'hrv_ods_view_data_cosucompany', label = '出口'))
  )
  return(res)
