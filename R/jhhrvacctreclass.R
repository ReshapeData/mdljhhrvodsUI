...
jhhrvacctreclassUI <- function(tabTitle ='重分类',
...
buttonacctreclassUI_left <- function() {
  ...
  res <- tagList(
    ...
    shiny::actionButton(inputId = 'btn_hrv_ods_view_acctreclass' , label = '预览表单数据'),
...
buttonacctreclassUI_right <- function() {
  res <- tagList(
    tsui::uiScrollX(tsui::mdl_dataTable(id = 'hrv_ods_view_data_acctreclass', label = '出口'))
  )
  return(res)
